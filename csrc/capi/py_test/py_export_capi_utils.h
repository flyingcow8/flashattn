...
inline Tensor_t convert_mcfa_tensor(const at::Tensor &at_tensor){
    ...
    if(dtype == torch::kFloat16)        mcft_dtype = MCFLASHATTN_DATATYPE_FP16;
    else if(dtype == torch::kBFloat16)  mcft_dtype = MCFLASHATTN_DATATYPE_BF16;
    else if(dtype == torch::kFloat)     mcft_dtype = MCFLASHATTN_DATATYPE_FP32;
    else if(dtype == torch::kDouble)    mcft_dtype = MCFLASHATTN_DATATYPE_FP64;
    else if(dtype == torch::kInt32)     mcft_dtype = MCFLASHATTN_DATATYPE_INT32;
    else if(dtype == torch::kInt64)     mcft_dtype = MCFLASHATTN_DATATYPE_INT64;
    else if(dtype == torch::kInt8)      mcft_dtype = MCFLASHATTN_DATATYPE_INT8;
    else assert(0);
