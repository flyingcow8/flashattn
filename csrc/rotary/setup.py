...
raise_if_cuda_home_none(PACKAGE_NAME)
...
def get_package_version():
    rotary_root = Path(__file__).parent.parent
    sha = get_sha(rotary_root)
    maca_version = os.environ.get("CORE_MODULE_MACA_VERSION")
    if maca_version:
        return "+metax" + maca_version
    elif sha != "Unknown":
        return "+git" + sha[:7]
    else:
        return ""
...
setup(
    name=PACKAGE_NAME,
    version="0.1" + get_package_version() + "torch" + get_torch_version() if get_torch_version() else get_package_version(),
