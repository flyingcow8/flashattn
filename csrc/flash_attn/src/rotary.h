...
__forceinline__ __device__ void copy_rotary_interleaved(Tensor<Engine0, Layout0> const &S, Tensor<Engine1, Layout1> &D,
    ...
    CUTE_STATIC_ASSERT_V(size<1>(S) == size<1>(Cos));
    CUTE_STATIC_ASSERT_V(size<2>(S) == size<2>(Cos));
    CUTE_STATIC_ASSERT_V(size<1>(S) == size<1>(Sin));
    CUTE_STATIC_ASSERT_V(size<2>(S) == size<2>(Sin));
    CUTE_STATIC_ASSERT_V(size<0>(Cos) == size<0>(Sin));
    static_assert(decltype(size<0>(S))::value == decltype(size<0>(Cos))::value * 2);
    static_assert(decltype(size<0>(Cos))::value % 2 == 0);
...
template <bool Is_even_K = true, bool Clear_OOB_K = true, typename Engine0, typename Layout0, typename Engine1,
          typename Layout1, typename Engine2, typename Layout2, typename Engine3, typename Layout3>
__forceinline__ __device__ void copy_rotary_contiguous(Tensor<Engine0, Layout0> const &S, Tensor<Engine1, Layout1> &D,
                                                       Tensor<Engine2, Layout2> const &Cos,
                                                       Tensor<Engine2, Layout2> const &Sin,
                                                       Tensor<Engine3, Layout3> const &identity_MN, const int max_MN,
                                                       const int min_MN, const int dim, const int rotary_dim) {
    ...
    CUTE_STATIC_ASSERT_V(size<1>(S) == size<1>(Sin));
    CUTE_STATIC_ASSERT_V(size<2>(S) == size<2>(Sin));
    CUTE_STATIC_ASSERT_V(size<0>(S) == size<0>(Cos));
    CUTE_STATIC_ASSERT_V(size<0>(Cos) == size<0>(Sin));
    static_assert(decltype(size<0>(Cos))::value % 2 == 0);
    Tensor rCos = make_fragment_like(Cos);
    Tensor rSin = make_fragment_like(Sin);
