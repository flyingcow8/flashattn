...
inline __device__ void compute_dq_dk_dv_1colblock_hdim64_64x64_8waves(const Params &params, const int bidb,
    ...
    typename Kernel_traits::GmemTiledCopyKV_hdim64_8wave gmem_tiled_copy_KV;
    ...
    auto smem_tiled_copy_PdS = make_tiled_copy_C(typename Kernel_traits::SmemCopyB64{}, tiled_mma_sdp);
    ...
    Tensor tdKrdSt = thr_mma_dkv.partition_fragment_A(sdSt);
    Tensor tdKrQt = thr_mma_dkv.partition_fragment_B(sQtNoSwizzle);

    Tensor tdVrPt = thr_mma_dkv.partition_fragment_A(sPtNoSwizzle);
    Tensor tdVrdOt = thr_mma_dkv.partition_fragment_B(sdOtransposedNoSwizzle);
...
#pragma unroll
    ...
    flash::copy_4x4_hdmi64<Is_even_MN, Is_even_K>(gmem_tiled_copy_KV, tVgV, tVrV, tKgK, tKrK, tKVcKV, tKVpKV,
                                                  binfo.actual_seqlen_k - n_block * kBlockN);
    ...
    Tensor tdStsdSt =
        make_tensor(make_smem_ptr(dStsmem_ptr_sts), make_layout(Shape<_4, _1, _2>{}, Stride<_1, _0, Int<32 * 64>>{}));
    Element *Ptsmem_ptr_sts = reinterpret_cast<Element *>(sP.data().get()) + warp_id * 4 * 64 + row * 64 + 4 * col;
    Tensor tPtsPt =
        make_tensor(make_smem_ptr(Ptsmem_ptr_sts), make_layout(Shape<_4, _1, _2>{}, Stride<_1, _0, Int<32 * 64>>{}));
    ...
    Element *dStsmem_ptr_lds =
        reinterpret_cast<Element *>(sdS.data().get()) + warp_id % 2 * 32 * 64 + lds_row * 64 + lds_col * 4;
    Tensor tdKsdSt = make_tensor(make_smem_ptr(dStsmem_ptr_lds),
                                 make_layout(Shape<_4, _2, _4>{}, Stride<_1, Int<16 * 64>, Int<4 * 64>>{}));
    Element *Ptsmem_ptr_lds =
        reinterpret_cast<Element *>(sP.data().get()) + warp_id % 2 * 32 * 64 + lds_row * 64 + lds_col * 4;
    Tensor tdVsPt = make_tensor(make_smem_ptr(Ptsmem_ptr_lds),
                                make_layout(Shape<_4, _2, _4>{}, Stride<_1, Int<16 * 64>, Int<4 * 64>>{}));
    ...
    Tensor tdKsQt =
        make_tensor(make_smem_ptr(Qtsmem_ptr_lds), make_layout(Shape<_4, _1, _4>{}, Stride<_1, _0, Int<16 * 64>>{}));
    Element *dOtsmem_ptr_lds = reinterpret_cast<Element *>(sdO.data().get()) + ldsQ_row * 64 + ldsQ_col * 4;
    Tensor tdVsdOt =
        make_tensor(make_smem_ptr(dOtsmem_ptr_lds), make_layout(Shape<_4, _1, _4>{}, Stride<_1, _0, Int<16 * 64>>{}));

    cute::copy(gmem_tiled_copy_KV, tVrV, tVsV);
    cute::copy(gmem_tiled_copy_KV, tKrK, tKsK);

    Tensor tKrK_reshape =
        make_tensor(tKrK.data(), make_layout(make_shape(size<0, 0>(tKrK), size<0, 1>(tKrK), size<2>(tKrK))));
    ...
    *(reinterpret_cast<cute::uint128_t *>(skt_sts_ptr)) = *(reinterpret_cast<cute::uint128_t *>(tKtrKt.data()));
    ...
    flash::copy_without_clear<Is_even_MN, Is_even_K, true>(gmem_tiled_copy_dO, tdOgdO, tdOrdO, tQcQ, tQpQ,
    ...
    clear(tQrQ);
    flash::copy_without_clear<Is_even_MN, Is_even_K, true>(gmem_tiled_copy_QKV, tQgQ, tQrQ, tQcQ, tQpQ,
                                                           binfo.actual_seqlen_q - m_block * kBlockM);
...
#pragma unroll
        ...
        flash::gemm_opt<false, Kernel_traits::Is_K_in_regs>(acc_s, tSrQ, tSrK, tSsQ, tSsK, tiled_mma_sdp,
        ...
        if constexpr (Has_attn_mask) {
            ...
            flash::apply_attn_mask(scores, n_block * kBlockN + (tidx / 64 / AtomLayoutMS) * MMA_N_SdP * 16,
                                   binfo.actual_seqlen_k, m_block * kBlockM + get<0>(taccScS_row(0)),
                                   binfo.actual_seqlen_q, AtomLayoutMS * 16, 16, params.scale_softmax, bias_ptr,
...
#pragma unroll
            for (int mi = 0; mi < size(lse); ++mi) {
                *(reinterpret_cast<uint32_t *>(&lse(mi))) =
                    __builtin_mxc_load_global_async32(reinterpret_cast<uint32_t *>(&gLSE(get<0>(taccScS_row(mi)))));
            }
...
#pragma unroll
        ...
        clear(acc_dq);
        flash::gemm_opt<false, Kernel_traits::Is_K_in_regs>(acc_dq, tdQrdS, tdQrKt, tdQsdS, tdQsKt, tiled_mma_dq,
...
#pragma unroll
        ...
        if (m_block > m_block_min) {
            ...
            flash::copy_without_clear<true, Is_even_K>(gmem_tiled_copy_dO, tdOgdO, tdOrdO, tQcQ, tQpQ);
            flash::copy_without_clear<true, Is_even_K>(gmem_tiled_copy_QKV, tQgQ, tQrQ, tQcQ, tQpQ);
        }
...
#pragma unroll
    ...
    auto smem_tiled_copy_dKV = make_tiled_copy_C(typename Kernel_traits::SmemCopyB64{}, tiled_mma_dkv);
    ...
    Tensor tdKsdK = make_tensor(make_smem_ptr(dKsmem_ptr),
                                make_layout(Shape<Shape<_1, _8>, _1, _1>{}, Stride<Stride<_0, _1>, _0, _0>{}));
    Tensor tdKgdK = gmem_thr_copy_dKV.partition_D(gdK);

    Element *dVsmem_ptr = reinterpret_cast<Element *>(sdV.data().get()) + (warp_id_new * 64 + lane_id) * 8;
    Tensor tdVsdV = make_tensor(make_smem_ptr(dVsmem_ptr),
                                make_layout(Shape<Shape<_1, _8>, _1, _1>{}, Stride<Stride<_0, _1>, _0, _0>{}));
    Tensor tdVgdV = gmem_thr_copy_dKV.partition_D(gdV);
...
#pragma unroll
    ...
    flash::copy_without_clear<Is_even_MN, Is_even_K, false, false>(gmem_tiled_copy_dKV, tdKrdK, tdKgdK, tdKVcdKV,
                                                                   tdKVpdKV, binfo.actual_seqlen_k - n_block * kBlockN);
    flash::copy_without_clear<Is_even_MN, Is_even_K, false, false>(gmem_tiled_copy_dKV, tdVrdV, tdVgdV, tdKVcdKV,
                                                                   tdKVpdKV, binfo.actual_seqlen_k - n_block * kBlockN);
}
