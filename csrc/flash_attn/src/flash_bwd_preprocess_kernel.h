...
inline __device__ void compute_dot_do_o(const Params &params) {
    ...
    constexpr int kNthreads =
        (Kernel_traits::kBlockM == 32 && Kernel_traits::kNThreads == 512) ? 256 : Kernel_traits::kNThreads;
...
#pragma unroll
    ...
    dot_do_o<Kernel_traits::kGmemThreadsPerRow>(tdOrdO, tdOrO, dP_sum, kNthreads / (Kernel_traits::kGmemThreadsPerRow),
                                                params.p_dropout);
    if (Clear_dQaccum) {
