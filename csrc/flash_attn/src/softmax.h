...
struct Softmax {
    ...
    template <bool Is_first, bool Check_inf = false, bool Syncthreads = false, bool AddVec = false, typename Tensor0,
              typename Tensor1>
    __forceinline__ __device__ void softmax_rescale_o(Tensor0 &acc_s, Tensor1 &acc_o, float softmax_scale_log2) {
        ...
        typedef __NATIVE_VECTOR__(2, float) Float2;
        if (Is_first) {
            flash::template thread_reduce_<true>(scores, row_max, max_op);
...
#pragma unroll
                ...
                SumOp<float> sum_op;
                flash::thread_reduce_<true>(scores, row_sum, sum_op);
            }
        } else {
            Tensor scores_max_prev = make_fragment_like(row_max);
            cute::copy(row_max, scores_max_prev);

            flash::template thread_reduce_<false>(scores, row_max, max_op);
