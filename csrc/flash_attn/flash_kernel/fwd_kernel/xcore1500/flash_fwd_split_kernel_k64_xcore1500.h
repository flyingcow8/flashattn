...
__forceinline__ __device__ void compute_attn_1rowblock_splitkv_k64(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {
    ...
    Tensor acc_o = partition_fragment_C(thr_mma_o, Shape<Int<kBlockM>, Int<kHeadDim>>{});  // MMA, MMA_M, MMA_K
    ...
    if constexpr (Append_KV) {
        ...
        Tensor tKgK_ = gmem_thr_copy_KV_new.partition_S(gK);  // (KCPY, KCPY_N, KCPY_K)
        Tensor tVgV_ = gmem_thr_copy_KV_new.partition_S(gV);  // (VCPY, VCPY_N, VCPY_K)
        Tensor tKgK_append = make_tensor(tKgK_.data(), reshape_thread_tile(tKgK_.layout()));
        Tensor tVgV_append = make_tensor(tVgV_.data(), reshape_thread_tile(tVgV_.layout()));
        Tensor tKcK_ = gmem_thr_copy_KV_new.partition_S(cK);   // (BCPY,BCPY_N,BCPY_K) -> (blk_n,blk_k)
        Tensor tVcV_ = gmem_thr_copy_KV_new.partition_S(cV);   // (BCPY,BCPY_N,BCPY_K) -> (blk_n,blk_k)
        Tensor tVcV_append = make_tensor(tVcV_.data(), reshape_thread_tile(tVcV_.layout()));
        Tensor tKcK_append = make_tensor(tKcK_.data(), reshape_thread_tile(tKcK_.layout()));
        if constexpr (Is_page_attn) {
            tVgV_append.data() = gV.data() + flash::resolve_thread_kv_page_slice_offset<Kernel_traits>(tidx, n_block_max, params.page_block_size,
                block_table, params.v_batch_stride, params.v_row_stride);
            tKgK_append.data() = gK.data() + flash::resolve_thread_kv_page_slice_offset<Kernel_traits>(tidx, n_block_max, params.page_block_size,
        ...
        auto tKgK_data = tKgK_append.data();
        auto tVgV_data = tVgV_append.data();
        for (int n_block = n_block_max - 1; n_block >= n_block_copy_min; n_block--) {
            flash::copy_w_min_idx<Is_even_K>(
                tVgVnew, tVgV_append, tVcV_append, params.d, binfo.actual_seqlen_k - n_block * kBlockN, binfo.seqlen_k_cache - n_block * kBlockN
            );
            tVgVnew.data() = tVgVnew.data() + (-int(kBlockN * params.vnew_row_stride));
            if (params.rotary_dim == 0) {
                flash::copy_w_min_idx<Is_even_K>(
                    tKgKnew, tKgK_append, tKcK_append, params.d, binfo.actual_seqlen_k - n_block * kBlockN, binfo.seqlen_k_cache - n_block * kBlockN
                );
            } else {
                if (params.is_rotary_interleaved) {
                    // Don't clear OOB_K because we're writing to global memory
                    flash::copy_rotary_interleaved<Is_even_K, /*Clear_OOB_K=*/false>(
                        tKgKnew, tKgK_append, tRgCos, tRgSin, tKcK_append, binfo.actual_seqlen_k - n_block * kBlockN,
                    ...
                    flash::copy_rotary_contiguous<Is_even_K, /*Clear_OOB_K=*/false>(
                        tKgKnew, tKgK_append, tRgCosCont, tRgSinCont, tKcK_append, binfo.actual_seqlen_k - n_block * kBlockN,
                        binfo.seqlen_k_cache - n_block * kBlockN, params.d, params.rotary_dim
                    );
            ...
            if constexpr (!Is_page_attn) {
                ...
                if (n_block > n_block_copy_min) {
                    tVgV_append.data() = gV.data() + flash::resolve_thread_kv_page_slice_offset<Kernel_traits>(tidx, n_block, params.page_block_size,
                        block_table, params.v_batch_stride, params.v_row_stride);
                    tKgK_append.data() = gK.data() + flash::resolve_thread_kv_page_slice_offset<Kernel_traits>(tidx, n_block, params.page_block_size,
                        block_table, params.k_batch_stride, params.k_row_stride);
                }
            }
        }
        ...
        barrier_gvm<0>();
        tKgK_append.data() = tKgK_data;
        tVgV_append.data() = tVgV_data;
    }
    ...
    if constexpr (!Is_page_attn) {
        ...
        flash::copy_page<Kernel_traits>(tKgK, page_idx, page_offset, n_block, block_table, params.k_batch_stride, params.k_row_stride, params.page_block_size);
        flash::copy_b128_page_bsm_async<Kernel_traits, Is_even_MN, Is_even_K>(gK, tKgK, tKsK, tKcK, params.d, n_block,
                                                                            block_table, params.k_batch_stride, params.k_row_stride, params.page_block_size, page_idx, page_offset, swz333_offset, binfo.actual_seqlen_k - n_block * kBlockN);
    }
    ...
    // Advance gV page offset
    if constexpr (Is_page_attn) {
        flash::copy_page<Kernel_traits>(tVgV, page_idx, page_offset, n_block, block_table, params.v_batch_stride, params.v_row_stride, params.page_block_size);
    }
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        if (masking_step > 0) {
            if constexpr (!Is_page_attn) {
                ...
                flash::copy_b128_bsm_async</*Is_even_MN=*/true, Is_even_K>(tVgV, tVsV, tVcV, params.d);
            } else {
                flash::copy_b128_page_bsm_async<Kernel_traits, /*Is_even_MN=*/true, Is_even_K>(gV, tVgV, tVsV, tVcV, params.d, n_block,
                                                                            block_table, params.v_batch_stride, params.v_row_stride, params.page_block_size, page_idx, page_offset, swz242_offset);
            }
        } else {
            if constexpr (!Is_page_attn) {
                // Clear the smem tiles to account for predicated off loads
                flash::copy_b128_bsm_async<Is_even_MN, Is_even_K>(tVgV, tVsV, tVcV, params.d, binfo.actual_seqlen_k - n_block * kBlockN);
            } else {
                flash::copy_b128_page_bsm_async<Kernel_traits, Is_even_MN, Is_even_K>(gV, tVgV, tVsV, tVcV, params.d, n_block,
                                                                            block_table, params.v_batch_stride, params.v_row_stride, params.page_block_size, page_idx, page_offset, swz242_offset, binfo.actual_seqlen_k - n_block * kBlockN);
            }
        }

        // Advance gK page offset
        if constexpr (Is_page_attn) {
            if(n_block > n_block_min) {
                flash::copy_page<Kernel_traits>(tKgK, page_idx, page_offset, n_block - 1, block_table, params.k_batch_stride, params.k_row_stride, params.page_block_size);
            }
        }
        ...
        if (n_block > n_block_min) {
            if constexpr (!Is_page_attn) {
                ...
                flash::copy_b128_page_bsm_async<Kernel_traits, /*Is_even_MN=*/true, Is_even_K>(gK, tKgK, tKsK, tKcK, params.d, n_block - 1,
                                                                            block_table, params.k_batch_stride, params.k_row_stride, params.page_block_size, page_idx, page_offset, swz333_offset);
            }
        }

        // We have key_padding_mask so we'll need to Check_inf
        masking_step == 0
            ? softmax.template softmax_rescale_o</*Is_first=*/true,  /*Check_inf=*/Is_causal || Is_local || !Is_even_MN, false, true>(acc_s, acc_o, params.scale_softmax_log2)
            : softmax.template softmax_rescale_o</*Is_first=*/false, /*Check_inf=*/Is_causal || Is_local || !Is_even_MN, false, true>(acc_s, acc_o, params.scale_softmax_log2);
        ...
        if constexpr (Is_page_attn) {
            if(n_block > n_block_min) {
                flash::copy_page<Kernel_traits>(tVgV, page_idx, page_offset, n_block - 1, block_table, params.v_batch_stride, params.v_row_stride, params.page_block_size);
            }
        }
    ...
    if (n_block == n_block_min) {
        ...
        softmax.template softmax_rescale_o</*Is_first=*/false, /*Check_inf=*/Is_local, false, true>(acc_s, acc_o, params.scale_softmax_log2);
    ...
    flash::copy_reg_to_global<Is_even_MN, Is_even_K>(
        tOrOaccum, tOgOaccum, tOcO, params.d, binfo.actual_seqlen_q - m_block * kBlockM
    );
}

template<typename Kernel_traits, bool Is_causal, bool Is_local, bool Has_alibi, bool Is_even_MN, bool Is_even_K, bool Is_softcap, bool Split, bool Append_KV, bool Is_page_attn, typename Params>
__forceinline__ __device__ void compute_attn_1rowblock_splitkv_k64_decode(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {
