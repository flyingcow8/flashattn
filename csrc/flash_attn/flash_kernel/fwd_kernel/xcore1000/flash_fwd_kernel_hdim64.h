...
__forceinline__ __device__ void compute_attn_1rowblock_opt_hdim64(const Params &params, const int bidb, const int bidh, const int m_block) {
    ...
    const index_t row_offset_mask = n_block * kBlockN * params.attn_mask_col_stride + kBlockM_stride * params.attn_mask_row_stride
    ...
    //For V we using ldg_b64 && sts_b64
    ...
    int tVcV[ldg_Num + (!Is_even_K * ldg_Num)];
    #pragma unroll
    for (int i = 0; i < ldg_Num; ++i) {
        ...
        const int old_sv_col = (laneId & 0x7) << 3;
        //tVsV_offset[i] = (sv_row - old_sv_row) * 64 + (sv_col - old_sv_col);
        tVsV_ptr[i] = reinterpret_cast<uint32_t *>(tVsV(_, _0{}, _0{}).data().ptr_ + ((sv_row - old_sv_row) << 6) + (sv_col - old_sv_col));
    ...
    if constexpr (Has_attn_mask) {
        flash::load_attn_mask<Merge_attn_mask_ldg, Is_even_M, Is_even_N>(tMaskgMask, tMaskrMask, tMaskcMask, seqlen_k_padded - n_block * kBlockN, binfo.actual_seqlen_q - kBlockM_stride);
    }
    flash::copy_global_to_reg_V<Is_even_MN, Is_even_K, /*Is_ldg_B128=*/false, ldg_Num>(
    ...
    // For performance reason, we separate out the first masking_step:
    // so we can prefech V from global mem eariler, and also reduce the number of cond-branch
    ...
    flash::copy_reg_to_share_V</*Is_sts_B128=*/false, ldg_Num>(tVrV, tVsV_ptr, perm_mask);
    ...
    softmax.template softmax_rescale_o</*Is_first=*/true,  /*Check_inf=*/Is_causal || Is_local || Has_attn_mask, /*Syncthreads=*/false, true>(acc_s, acc_o, params.scale_softmax_log2);
    ...
    const int block_row_idx = m_block * (kBlockM / 16) + tidx / 64 % kNWarps;
    ...
    flash::gemm_rs<Is_perm_4x4, lds_Tuple>(acc_o, rP, tOrVt, tOsVt, tiled_mma, cpy_offset);
    n_block--;
    ...
    for (int masking_step = 1; n_block >= n_block_min && masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        if constexpr (Has_attn_mask) {
            ...
            flash::load_attn_mask<Merge_attn_mask_ldg, Is_even_M, /*Is_even_N=*/true>(tMaskgMask, tMaskrMask, tMaskcMask, seqlen_k_padded - n_block * kBlockN, binfo.actual_seqlen_q - kBlockM_stride);
        }
        tVgV.data() = tVgV.data() + gV_offset;
        flash::copy_global_to_reg_V</*Is_even_MN=*/true, Is_even_K, /*Is_ldg_B128=*/false, ldg_Num>(tVgV, tVrV, tVcV, tVgV_offset, d);
        flash::sync_threads();
        ...
        softmax.template softmax_rescale_o</*Is_first=*/false, /*Check_inf=*/Is_causal || Is_local || Has_attn_mask, /*Syncthreads=*/false, true>(acc_s, acc_o, params.scale_softmax_log2);
    ...
    if constexpr(kBlockM == 64) {
        ...
        #pragma unroll 4
        for (int i = 0; i < 4; ++i) {
            auto ptr = reinterpret_cast<uint64_t *>(rO.data().ptr_ + (i << 2));
        ...
        for (int i = 0; i < 4; ++i) {
            const int col = ((((laneId & 0x7) ^ (((laneId >> 4) << 1) + (i >> 1))) << 1) + (i & 0x1)) << 2;
            auto ptr0 = reinterpret_cast<uint64_t *>(rO.data().ptr_ + (i << 3));
            auto ptr1 = reinterpret_cast<uint64_t *>(rO.data().ptr_ + (i << 3) + 4);
