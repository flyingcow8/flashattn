...
// support all page size include page_size=1
...
__forceinline__ __device__ void compute_attn_1rowblock_splitkv_k64_page_one(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {
    ...
    Tensor tVsV = make_tensor(make_smem_ptr(smem_v), make_layout(make_shape(size<0>(tVgV), size<1>(tVgV), size<2>(tVgV)),
                                                                 make_stride(_1{}, Int<16 * kNThreads>{}, Int<kBlockKGmem * kBlockN>{})));
    ...
    Element *smem_vt = reinterpret_cast<Element *>(sVt.data().get()) + __lane_id() / 16 * kBlockKSmem * 4 +
                                                  __lane_id() % 16 * 4;
    Tensor tOsVt = make_tensor(make_smem_ptr(smem_vt), make_layout(Shape <_4, Shape<_4, Int<kHeadDim / kBlockKSmem>>, Int<kBlockN / 16>>{}, // (MMA,MMA_N,MMA_K)
                                                                   Stride<_1, Stride<Int<kBlockKSmem>, Int<kBlockKSmem * kBlockN>>, Int<kBlockKSmem * 16>>{}));
    ...
    clear(acc_o);
    // We apply sink in combine kernel when Split=true to prevent the sink from being skipped.
    const bool has_sink = !Split && params.s_aux_ptr != nullptr;
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        if (masking_step > 0) {
            if constexpr (!Is_page_attn) {
                ...
                flash::copy_multirow_b64</*Is_even_MN=*/true, Is_even_K>(tVgV, tVrV, tVcV, params.d);
            } else {
                flash::copy_multirow_b64_page_one<Kernel_traits, /*Is_even_MN=*/true, Is_even_K>(gV, tVgV, tVrV, tVcV, params.d, n_block,
            ...
            if constexpr (!Is_page_attn) {
                ...
                flash::copy_multirow_b64_page_one<Kernel_traits, Is_even_MN, Is_even_K>(gV, tVgV, tVrV, tVcV, params.d, n_block,
...
// Support the page_size is a multiple of 4 like page_size = 4 or 16, but not support page_size=1
template<typename Kernel_traits, bool Is_causal, bool Is_local, bool Has_alibi, bool Is_even_MN, bool Is_even_K, bool Is_softcap, bool Split, bool Append_KV, bool Is_page_attn, typename Params>
__forceinline__ __device__ void compute_attn_1rowblock_splitkv_k64(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {
    ...
    typename Kernel_traits::GmemTiledCopyQKVPaged gmem_tiled_copy_K;
    typename Kernel_traits::GmemTiledCopyVPaged_4x4 gmem_tiled_copy_V;
    ...
    Tensor tKgK_ = gmem_thr_copy_K.partition_S(gK);  // (KCPY, KCPY_N, KCPY_K)
    Tensor tKsK_ = gmem_thr_copy_K.partition_D(sK);
    Tensor tVgV_ = gmem_thr_copy_V.partition_S(gV);  // (VCPY, VCPY_N, VCPY_K)
    ...
    Tensor tVrV = make_fragment_like(tVgV);
    Element *smem_v = reinterpret_cast<Element *>(sV.data().get()) + tidx * 16;
    static_assert(size<1>(tVgV) == _4{});
    Tensor tVsV = make_tensor(make_smem_ptr(smem_v), make_layout(make_shape(size<0>(tVgV)*size<1>(tVgV), _1{}, size<2>(tVgV)),
                                                                 make_stride(_1{}, _0{}, Int<kBlockKGmem * kBlockN>{})));
    ...
    if constexpr (Is_page_attn) {
        ...
        tVgV.data() = gV.data() + flash::resolve_thread_kv_page_slice_offset_b64<Kernel_traits>(tidx, n_block_max, params.page_block_size,
    ...
    Tensor tKVcKV_ = gmem_thr_copy_K.partition_S(cKV);   // (BCPY,BCPY_N,BCPY_K) -> (blk_n,blk_k)
    Tensor tVcV_ = gmem_thr_copy_V.partition_S(cKV);   // (BCPY,BCPY_N,BCPY_K) -> (blk_n,blk_k)
    ...
    if constexpr (Append_KV) {
        ...
        Tensor tVgVnew = gmem_thr_copy_K.partition_S(gVnew);  // (VCPY, VCPY_N, VCPY_K)
        Tensor tVgVcache = gmem_thr_copy_K.partition_S(gV);  // (VCPY, VCPY_N, VCPY_K)
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        Tensor acc_s = partition_fragment_C(tiled_mma, Shape<Int<kBlockM>, Int<kBlockN>>{});  // (MMA=4, MMA_M, MMA_N)
        cute::copy(gmem_tiled_copy_K, tKrK, tKsK);
        clear(acc_s);
        ...
        if (masking_step > 0) {
            if constexpr (!Is_page_attn) {
                ...
                tVgV.data() = gV.data() + flash::resolve_thread_kv_page_slice_offset_b64<Kernel_traits>(tidx, n_block + 1, params.page_block_size,
                    block_table, params.v_batch_stride, params.v_row_stride);
            }
            flash::copy_b64</*Is_even_MN=*/true, Is_even_K>(tVgV, tVrV, tVcV, params.d);
        } else {
            flash::copy_b64<Is_even_MN, Is_even_K>(tVgV, tVrV, tVcV, params.d, binfo.actual_seqlen_k - n_block * kBlockN);
        }
        flash::sync_threads();
        ...
        masking_step == 0
            ...
            : softmax.template softmax_rescale_o</*Is_first=*/false, /*Check_inf=*/Is_causal || Is_local || !Is_even_MN, false, true>(acc_s, acc_o, params.scale_softmax_log2);
        // if (cute::thread0()) { print(scores_max); print(scores_sum); print(scores); }
        permute_4x4_b16(tVrV);
        Tensor tVrV_view = make_tensor(tVrV.data(), make_layout(make_shape(size<0>(tVrV)*size<1>(tVrV), _1{}, size<2>(tVrV))));
