...
__forceinline__ __device__ void compute_attn_1rowblock_opt_hdim256(const Params &params, const int bidb, const int bidh, const int m_block) {
    ...
    for (int i = 0; i < ldg_Num; ++i) {
         ...
         ***********************************************************************/
        const int gv_row = kBlockN == 32 ? (((laneId >> 4) & 0x1) << 2) + (i & 0x3)
                                        : (((laneId >> 4) & 0x1) << 2) + ((laneId >> 5) << 5) + (i & 0x3);
        const int gv_col = kBlockN == 32 ? ((laneId & 0xf) << 2) + ((i >> 2) << 7) + ((laneId >> 5) << 6)
                                        : ((laneId & 0xf) << 3) + ((i >> 2) << 7);
        ...
        const int sv_row = kBlockN == 32 ? gv_row + ((laneId >> 5) << 5) + ((i >> 2) << 6)
                                         : gv_row + ((old_gv_row & 0x1) * kBlockN) + ((i >> 2) << 7);
        const int sv_col = kBlockN == 32 ? ((laneId & 0xf)) << 2
                                         : ((laneId & 0x7)) << 3;
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        flash::copy_reg_to_share_V</*Is_sts_B128=*/Is_B128, ldg_Num, Is_perm_4x4>(tVrV, tVsV_ptr, perm_mask);
...

    {
        ...
        #pragma unroll 4
        for(int i = 0; i < 4; ++i) {
            auto ptr = reinterpret_cast<uint32_t *>(rO.data().ptr_ + (i << 4));
        ...
        for(int i = 0; i < 4; ++i) {
            ...
            #pragma unroll 4
            for (int j = 0; j < 4; ++j) {
                auto ptr = reinterpret_cast<uint64_t *>(rO.data().ptr_ + (j << 4));
                auto sm_ptr = reinterpret_cast<uint64_t *>(s_ptr + new_sm_col) + (j << 10);
                sm_ptr[0] = ptr[i];
            }
        }
    }
