...
__forceinline__ __device__ void compute_attn_1rowblock_splitkv_opt_hdim96_kblockM64(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {
    ...
    if constexpr(!Split) {
        ...
        #pragma unroll
        for (int i = 0; i < 3; ++i) {
            auto ptr = reinterpret_cast<uint32_t *>(rO.data().ptr_ + (i << 3));
            auto a = __builtin_mxc_byte_perm(ptr[2], ptr[0], perm_mask[0]);
            ptr[2] = __builtin_mxc_byte_perm(ptr[2], ptr[0], perm_mask[1]);
            ptr[0] = a;
            auto b = __builtin_mxc_byte_perm(ptr[3], ptr[1], perm_mask[0]);
            ptr[3] = __builtin_mxc_byte_perm(ptr[3], ptr[1], perm_mask[1]);
            ptr[1] = b;
        ...
        for (uint32_t i = 0; i < 3; ++i) {
            auto ptr = reinterpret_cast<uint128_t *>(rO.data().ptr_ + (i << 3));
            uint32_t col = ((laneId >> 4) ^ (((laneId & 0xf) >> 1) & 0x3)) << 3;
            auto sm_ptr = reinterpret_cast<uint128_t *>(s_ptr + col + (i << 11));
            sm_ptr[0] = ptr[0];
        }
    }
    else {
        ...
        #pragma unroll
        for (uint32_t i = 0; i < 3; ++i) {
            auto ptr = reinterpret_cast<uint32_t *>(acc_o.data() + (i << 3));
            uint32_t col = ((laneId >> 4) ^ (((laneId & 0xf) >> 1) & 0x3)) << 3;
            auto sm_ptr = reinterpret_cast<uint32_t *>(s_ptr + col + (i << 11));
            sm_ptr[0] = ptr[0];
            sm_ptr[1] = ptr[4];
            sm_ptr[2] = ptr[1];
            sm_ptr[3] = ptr[5];
