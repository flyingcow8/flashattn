...
__forceinline__ __device__ void compute_attn_1rowblock_splitkv_opt_hdim128_kblockMsmall(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {
    ...
    constexpr bool Is_perm_4x4 = ldg_Num > 2;
    ...
    for (int i = 0; i < ldg_Num; ++i) {
         ...
         * gv_rows means laneId[0-63] read 0~15 rows with 4 groups
         * For kBlockN=32, ldg_num=2:
         * gv_rows means laneId[0-63] read 0~7 rows with 2 groups
         * For all kBlockN:
         * gv_col means [0~63] read 0~15th 8cols
         ***********************************************************************/
        const int gv_row = ldg_Num == 2 ? ((laneId >> 4) << 1) + i : ((laneId >> 4) << 2) + i;
        ...
        const int sv_row = gv_row + (((laneId >> 3) & 0x1) * kBlockN) + (tidx / 64) * (ldg_Num == 2 ? 8 : 16);
        const int& sv_col = old_gv_col;
    ...
    if constexpr(!Split) {
        const int sm_offset = (16 * 64 * kNWarps) / 4;//We asume every wave store 16 rows elements
        ...
        sm_ptr = reinterpret_cast<uint64_t *>(s_ptr + sm_col);
        sm_ptr[0] = ptr[2];
        sm_ptr[sm_offset] = ptr[6];
        sm_col = ((((laneId & 0x7) ^ (((laneId >> 4) << 1) + (3 >> 1))) << 1) + (3 & 0x1)) << 2;
        sm_ptr = reinterpret_cast<uint64_t *>(s_ptr + sm_col);
        sm_ptr[0] = ptr[3];
        sm_ptr[sm_offset] = ptr[7];
    }
    else {

        const int sm_offset = 16 * 64 * kNWarps;//We asume every wave store 16 rows elements
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            sm_ptr[0] = ptr0[i];
            sm_ptr[sm_offset] = ptr1[i];
            sm_ptr[1] = ptr0[i + 4];
            sm_ptr[sm_offset + 1] = ptr1[i + 4];
            sm_ptr[2] = ptr0[i + 8];
            sm_ptr[sm_offset + 2] = ptr1[i + 8];
            sm_ptr[3] = ptr0[i + 12];
            sm_ptr[sm_offset + 3] = ptr1[i + 12];
        }
    }
