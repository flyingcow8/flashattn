...
__forceinline__ __device__ void compute_attn_1rowblock_opt_hdim128_blockM64(const Params &params, const int bidb, const int bidh, const int m_block) {
...

    {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            auto ptr0 = reinterpret_cast<uint64_t *>(tSrQ_ptr + (i << 1));
            auto s_ptr0 = reinterpret_cast<uint64_t *>(tSsQ_ptr + col);
            auto ptr1 = reinterpret_cast<uint64_t *>(tSrQ_ptr + (i << 1) + 8);
            auto s_ptr1 = reinterpret_cast<uint64_t *>(tSsQ_ptr + col + 64 * 32);
            ptr0[0] = s_ptr0[0];
            ptr1[0] = s_ptr1[0];
        }
    }
    ...
    const auto mask_offset = kBlockM_stride + (tidx >> 6) * 16 + (tidx & 0xf);
