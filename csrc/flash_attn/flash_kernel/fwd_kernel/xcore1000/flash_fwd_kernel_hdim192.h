...
__forceinline__ __device__ void compute_attn_1rowblock_opt_hdim192(const Params &params, const int bidb, const int bidh, const int m_block) {
    ...
    constexpr int ldg_Num = 8;
    //Every lds_Tuple load 16 elements
    constexpr int lds_Tuple = 3;
    ...
    for (int i = 0; i < ldg_Num; ++i) {
         ...
         * gv_col means [0~63] read 0~15th 8cols for first 4-ldg_num, lastest read 4cols,
         * and for lastest 4-ldg_num, gv_col need to increase 16 8cols
         ***********************************************************************/
        const int gv_row = (((laneId >> 4) & 0x1) << 2) + ((laneId >> 5) << 5) + (i & 0x3);
        const int gv_col = i >= 4 ? ((laneId & 0xf) << 2) + 128
                                  : (laneId & 0xf) << 3;
        ...
        const int sv_row = i >= 4 ? gv_row + 128
                                  : gv_row + ((old_gv_row & 0x1) * kBlockN);
        const int sv_col = i >= 4 ? (laneId & 0xf) << 2
                                  : (laneId & 0x7) << 3;
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        if (masking_step > 0) {
            ...
            flash::copy_global_to_reg_V</*Is_even_MN=*/true, Is_even_K, /*Is_ldg_B128=*/2, ldg_Num>(tVgV, tVrV, tVcV, tVgV_offset, d);
        } else {
            // Clear the smem tiles to account for predicated off loads
            flash::copy_global_to_reg_V<Is_even_MN, Is_even_K,/*Is_ldg_B128=*/2, ldg_Num>(
        ...
        flash::copy_reg_to_share_V</*Is_sts_B128=*/2, ldg_Num, Is_perm_4x4>(tVrV, tVsV_ptr, perm_mask);
