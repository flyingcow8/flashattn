...
__forceinline__ __device__ void compute_attn_1rowblock_k64_tail_32(const Params &params, const int bidb, const int bidh, const int m_block) {
    ...
    constexpr int kBlockK_tail = Kernel_traits::kBlockKSmem;
    constexpr int kHeadDim_tail = 32;
    constexpr int kHeadDim_pre = kHeadDim - kHeadDim_tail;
    const int d_tail = params.d - kHeadDim_pre;
    constexpr bool Is_even_M = Is_even_MN;
    constexpr bool Is_even_N = Is_even_MN;

    static_assert(kBlockK_tail == 32);
    ...
    Tensor gV = make_tensor(make_gmem_ptr(reinterpret_cast<Element *>(params.v_ptr) + row_offset_v),
                            Shape<Int<kBlockN>, Int<kHeadDim_pre>>{},
                            make_stride(params.v_row_stride, _1{}));
    Tensor gV_tail = make_tensor(make_gmem_ptr(reinterpret_cast<Element *>(params.v_ptr) + row_offset_v + kHeadDim_pre),
                            Shape<Int<kBlockN>, Int<kHeadDim_tail>>{},
                            make_stride(params.v_row_stride, _1{}));
    ...
    Tensor sV = make_tensor(sK.data() + size(sK), typename Kernel_traits::SmemLayoutVtNoSwizzle_pre{});
    Tensor sVt = make_tensor(sV.data(), typename Kernel_traits::SmemLayoutVtNoSwizzle_pre{});
    Tensor sVtNoSwizzle = make_tensor(sV.data(), typename Kernel_traits::SmemLayoutVtransposedNoSwizzle_pre{});
    Tensor sV_tail = make_tensor(sV.data() + size(sV), typename Kernel_traits::SmemLayoutVtNoSwizzle_tail{});
    Tensor sVt_tail = make_tensor(sV_tail.data(), typename Kernel_traits::SmemLayoutVtNoSwizzle_tail{});
    Tensor sVtNoSwizzle_tail = make_tensor(sV_tail.data(), typename Kernel_traits::SmemLayoutVtransposedNoSwizzle_tail{});
    ...
    typename Kernel_traits::GmemTiledCopy_4x2 gmem_tiled_copy_V_tail;
    typename Kernel_traits::GmemTiledCopy_4x4 gmem_tiled_copy_V;
    auto gmem_thr_copy_V = gmem_tiled_copy_V.get_thread_slice(tidx);
    auto gmem_thr_copy_V_tail = gmem_tiled_copy_V_tail.get_thread_slice(tidx);
    Tensor tVgV = gmem_thr_copy_V.partition_S(gV);  // (VCPY, VCPY_N, VCPY_K)
    Tensor tVrV = make_fragment_like(tVgV);
    Tensor tVgV_tail = gmem_thr_copy_V_tail.partition_S(gV_tail);  // (VCPY, VCPY_N, VCPY_K)
    Tensor tVrV_tail = make_fragment_like(tVgV_tail);
    Tensor tVrV_view = make_tensor(tVrV.data(), make_layout(make_shape(size<0, 0>(tVrV), size<0, 1>(tVrV), size<1>(tVrV)*size<2>(tVrV))));
    Tensor tVrV_tail_view = make_tensor(tVrV_tail.data(), make_layout(make_shape(size<0>(tVrV_tail), size<1>(tVrV_tail), size<2>(tVrV_tail))));
    ...
    Tensor tVsV = make_tensor(make_smem_ptr(smem_v), make_layout(make_shape(size<0>(tVgV), size<1>(tVgV), size<2>(tVgV)),
                                                                 make_stride(_1{}, Int<16 * kNThreads>{}, Int<kBlockK * kBlockN>{})));
    Element *smem_v_tail = reinterpret_cast<Element *>(sV_tail.data().get()) + tidx * 8;
    Tensor tVsV_tail = make_tensor(make_smem_ptr(smem_v_tail), make_layout(Shape <_8, Int<kBlockN / kNWarps / 16>, Int<kHeadDim_tail / kBlockK_tail>>{},
                                                                 Stride<_1, Int<kNWarps * 16 * kBlockK_tail>, Int<kBlockN * kBlockK_tail>>{}));
    ...
    Tensor tOrVt_tail  = thr_mma_o.partition_fragment_B(sVtNoSwizzle_tail);                // (MMA, MMA_K,MMA_N)
    ...
    Element *smem_vt = reinterpret_cast<Element *>(sVt.data().get()) + __lane_id() / 16 * kBlockK * 4 +
                                                  __lane_id() % 16 * 4;
    Tensor tOsVt = make_tensor(make_smem_ptr(smem_vt), make_layout(Shape <_4, Shape<_4, Int<kHeadDim_pre / kBlockK>>, Int<kBlockN / 16>>{}, // (MMA,MMA_N,MMA_K)
                                                                   Stride<_1, Stride<Int<kBlockK>, Int<kBlockK * kBlockN>>, Int<kBlockK * 16>>{}));
    Element *smem_vt_tail = reinterpret_cast<Element *>(sVt_tail.data().get()) + __lane_id() / 16 * kBlockK_tail * 4 +
                                                  __lane_id() % 16 / 8 * kBlockK_tail + __lane_id() % 8 * 4;
    Tensor tOsVt_tail = make_tensor(make_smem_ptr(smem_vt_tail), make_layout(Shape <_4, Shape<_2, Int<kHeadDim_tail / kBlockK_tail>>, Int<kBlockN / 16>>{}, // (MMA,MMA_N,MMA_K)
                                                                   Stride<_1, Stride<Int<kBlockK_tail * 2>, Int<kBlockK_tail * kBlockN>>, Int<kBlockK_tail * 16>>{}));
    ...
    Tensor cV_tail = make_identity_tensor(make_shape(size<0>(sV_tail), size<1>(sV_tail)));    // (BLK_N,BLK_K) -> (blk_n,blk_k)

    // Repeat the partitioning with identity layouts
    Tensor tQcQ_Noswizzle = gmem_thr_copy_QK.partition_S(cQ);       // (ACPY,ACPY_M,ACPY_K) -> (blk_m,blk_k)
    Tensor tKcK_Noswizzle = gmem_thr_copy_QK.partition_S(cK);   // (BCPY,BCPY_N,BCPY_K) -> (blk_n,blk_k)
    Tensor tVcV = gmem_thr_copy_V.partition_S(cV);    // (BCPY,BCPY_N,BCPY_K) -> (blk_n,blk_k)
    Tensor tVcV_tail = gmem_thr_copy_V_tail.partition_S(cV_tail);    // (BCPY,BCPY_N,BCPY_K) -> (blk_n,blk_k)
    ...
    constexpr int ldg_V_cnt_pre = size<0, 1>(tVgV) * size<1>(tVgV) * size<2>(tVgV);
    constexpr int ldg_V_cnt_tail = size<0, 1>(tVgV_tail) * size<1>(tVgV_tail) * size<2>(tVgV_tail);
    constexpr int ldg_V_cnt_tot = ldg_V_cnt_pre + ldg_V_cnt_tail;
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        if (masking_step > 0) {
            ...
            tVgV_tail.data() = tVgV_tail.data() + (-int(kBlockN * params.v_row_stride));
            flash::copy_multirow_b64</*Is_even_MN=*/true, /*Is_even_K=*/true, /*is_async*/true>(tVgV, tVrV, tVcV, params.d);
            flash::copy_multirow_b32</*Is_even_MN=*/true, Is_even_K, /*is_async*/true>(tVgV_tail, tVrV_tail, tVcV_tail, d_tail);
        } else {
            flash::copy_multirow_b64<Is_even_MN, /*Is_even_K=*/true, /*is_async*/true>(
                tVgV, tVrV, tVcV, params.d, binfo.actual_seqlen_k - n_block * kBlockN);
            flash::copy_multirow_b32<Is_even_MN, Is_even_K, /*is_async*/true>(
                tVgV_tail, tVrV_tail, tVcV_tail, d_tail, binfo.actual_seqlen_k - n_block * kBlockN);
        }
        ...
        cute::copy(tVrV, tVsV);
        permute_4x2_b16(tVrV_tail);
        cute::copy(tVrV_tail_view, tVsV_tail);
        ...
        flash::sync_threads();
        flash::gemm_rs(acc_o, tOrP, tOrVt, tOsVt, tOrVt_tail, tOsVt_tail, tiled_mma_o, smem_tiled_copy_V, smem_thr_copy_V);
    ...
    for (; n_block > n_block_min; --n_block) {
        ...
        flash::copy_b128_bsm_async</*Is_even_MN=*/true, Is_even_K>(tKgK, tKsK, tKcK, params.d);
        __builtin_mxc_schedbound_end();
        cp_async_wait<ldg_V_cnt_tail + ldg_K_cnt>();
        permute_4x4_b16(tVrV_view);
        cute::copy(tVrV, tVsV);
