...
__forceinline__ __device__ void compute_attn_1rowblock_splitkv_opt_hdim128_kblockM128(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {
    ...
    if constexpr(!Split) {
        ...
        #pragma unrool 2
        for (int i = 0; i < 2; ++i) {
            auto ptr = acc_o.data() + 4 * i;
            #pragma unroll 4
            for (int j = 0; j < 4; ++j) {
                int new_sm_col = ((((laneId & 0x7) ^ (((laneId >> 4) << 1) + (j >> 1))) << 1) + (j & 0x1)) << 2;
                auto sm_ptr = s_ptr + new_sm_col + 64 * 64 * i;
                sm_ptr[0] = ptr[j];
                sm_ptr[1] = ptr[j + 8];
                sm_ptr[2] = ptr[j + 16];
                sm_ptr[3] = ptr[j + 24];
