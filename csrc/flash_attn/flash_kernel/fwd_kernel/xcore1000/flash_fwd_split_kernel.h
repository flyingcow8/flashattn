...
#include "flash_fwd_split_kernel_hdim64.h"
#include "flash_fwd_split_kernel_hdim96_kBlockM64.h"
#include "flash_fwd_split_kernel_hdim96_kBlockM128.h"
#include "flash_fwd_split_kernel_hdim128_kBlockM128.h"
#include "flash_fwd_split_kernel_hdim128_kBlockMsmall.h"
#include "flash_fwd_split_kernel_hdim256.h"

#include "flash_fwd_split_kernel_k32.h"
#include "flash_fwd_split_kernel_k64_dequant.h"
#include "flash_fwd_split_kernel_k64.h"
...
__forceinline__ __device__ void compute_attn_1rowblock_splitkv(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {
    ...
    if (Is_page_attn && params.page_block_size == 1) {
        flash::copy_page_one<Kernel_traits, Is_even_MN, Is_even_K>(gmem_tiled_copy_KV, tKgK, tKsK, tKVcKV, tKVpKV,
                                                    gK, n_block_max, block_table, params.k_batch_stride, params.k_row_stride,
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        if (masking_step > 0) {
            if constexpr(!Is_page_attn) {
                ...
                if (params.page_block_size == 1) {
                    flash::copy_page_one<Kernel_traits, true, Is_even_K>(gmem_tiled_copy_KV, tVgV, tVsV, tKVcKV, tKVpKV,
                        gV, n_block + 1, block_table, params.v_batch_stride, params.v_row_stride);
                } else {
            ...
            if (Is_page_attn && params.page_block_size == 1) {
                flash::copy_page_one<Kernel_traits, Is_even_MN, Is_even_K, true>(gmem_tiled_copy_KV, tVgV, tVsV, tKVcKV, tKVpKV,
                    gV, n_block_max, block_table, params.v_batch_stride, params.v_row_stride, binfo.actual_seqlen_k - n_block * kBlockN);
            } else {
                flash::copy<Is_even_MN, Is_even_K, /*Clear_OOB_MN=*/true>(gmem_tiled_copy_KV, tVgV, tVsV,
        ...
        if (n_block > n_block_min) {
            ...
            if constexpr(!Is_page_attn) {
                ...
                if (params.page_block_size == 1) {
                    flash::copy_page_one<Kernel_traits, true, Is_even_K>(gmem_tiled_copy_KV, tKgK, tKsK, tKVcKV, tKVpKV,
                        gK, n_block, block_table, params.k_batch_stride, params.k_row_stride);
                } else {
                    tKgK.data() = gK.data() + flash::resolve_thread_kv_page_slice_offset<Kernel_traits>(tidx, n_block, params.page_block_size,
                        block_table, params.k_batch_stride, params.k_row_stride);
                    flash::copy</*Is_even_MN=*/true, Is_even_K>(gmem_tiled_copy_KV, tKgK, tKsK, tKVcKV, tKVpKV);
    ...
    if constexpr (Split || Kernel_traits::Share_Q_K_smem) { __syncthreads(); }
...
__forceinline__ __device__ void compute_attn_splitkv(const Params &params, const int m_block_max, const int& block_type) {
    ...
    if constexpr (Kernel_traits::kHeadDim == 128 && Is_page_attn) {
        if (params.dequant_group > 0) {
            compute_attn_1rowblock_splitkv_k64_dequant<Kernel_traits, Is_causal, Is_local, Has_alibi, Is_even_MN,
                                                            Is_even_K, Is_softcap, Split, Append_KV, Is_page_attn>(
                params, bidb, bidh, m_block, n_split_idx, num_n_splits);
            return;
        }
        if constexpr (!Append_KV) {
            if (params.page_block_size == 1) {
                compute_attn_1rowblock_splitkv_k64_page_one<Kernel_traits, Is_causal, Is_local, Has_alibi, Is_even_MN,
                                                        Is_even_K, Is_softcap, Split, Append_KV, Is_page_attn>(
                    params, bidb, bidh, m_block, n_split_idx, num_n_splits);
                return;
            }
    ...
    if constexpr ((Kernel_traits::kHeadDim != 64) && (Kernel_traits::kHeadDim != 128) && (Kernel_traits::kHeadDim != 256) && Is_page_attn) {
        if (params.page_block_size % 256 != 0) {
            compute_attn_1rowblock_splitkv<Kernel_traits, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_K,
                                                    Is_softcap, Split, Append_KV, true>(params, bidb, bidh, m_block,
    ...
    if constexpr (Kernel_traits::kHeadDim == 128) {
        ...
        if constexpr (Kernel_traits::kBlockM <= 64) {
            compute_attn_1rowblock_splitkv_opt_hdim128_kblockMsmall<Kernel_traits, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_K, Is_softcap, Split, Append_KV, Is_page_attn>(params, bidb, bidh, curr_block, n_split_idx, num_n_splits);
        }else {
            compute_attn_1rowblock_splitkv_opt_hdim128_kblockM128<Kernel_traits, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_K, Is_softcap, Split, Append_KV, Is_page_attn>(params, bidb, bidh, curr_block, n_split_idx, num_n_splits);
        }
    } else if constexpr (Kernel_traits::kHeadDim == 256) {
        // Note: only support 64x32  Is_split=false Split=true
        if constexpr (Kernel_traits::kBlockM == 64 && Kernel_traits::kBlockN == 32 && Split) {
            if (params.page_block_size != 1 && (params.page_block_size / 4) % 2 == 0) {
                compute_attn_1rowblock_splitkv_opt_hdim256<Kernel_traits, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_K, Is_softcap, Split, Append_KV, Is_page_attn>(params, bidb, bidh, m_block, n_split_idx, num_n_splits);
            } else {
                compute_attn_1rowblock_splitkv<Kernel_traits, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_K, Is_softcap, Split, Append_KV, Is_page_attn>(params, bidb, bidh, m_block, n_split_idx, num_n_splits);
            }
        } else {
            compute_attn_1rowblock_splitkv<Kernel_traits, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_K, Is_softcap, Split, Append_KV, Is_page_attn>(params, bidb, bidh, m_block, n_split_idx, num_n_splits);
        }
        ...
        const int curr_block = m_block_max - m_block - 1;
        if constexpr (Kernel_traits::kBlockM == 16 && Kernel_traits::kBlockN == 16 && !Append_KV) {
            // not support append_kv
            compute_attn_1rowblock_splitkv_k64<Kernel_traits, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_K, Is_softcap, Split, Append_KV, Is_page_attn>(params, bidb, bidh, m_block, n_split_idx, num_n_splits);
        } else {
            compute_attn_1rowblock_splitkv_opt_hdim64<Kernel_traits, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_K, Is_softcap, Split, Append_KV, Is_page_attn>(params, bidb, bidh, curr_block, n_split_idx, num_n_splits);
        }
    } else if constexpr (Kernel_traits::kHeadDim == 96) {
        const int curr_block = m_block_max - m_block - 1;
        if constexpr (Kernel_traits::kBlockM == 64) {
            compute_attn_1rowblock_splitkv_opt_hdim96_kblockM64<Kernel_traits, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_K, Is_softcap, Split, Append_KV>(params, bidb, bidh, curr_block, n_split_idx, num_n_splits);
        } else {
            compute_attn_1rowblock_splitkv_opt_hdim96_kblockM128<Kernel_traits, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_K, Is_softcap, Split, Append_KV>(params, bidb, bidh, curr_block, n_split_idx, num_n_splits);
        }
    } else if constexpr (Kernel_traits::kHeadDim == 32 || Kernel_traits::kHeadDim == 160){
        compute_attn_1rowblock_splitkv_k32<Kernel_traits, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_K, Is_softcap, Split, Append_KV>(params, bidb, bidh, m_block, n_split_idx, num_n_splits);
    } else {
        compute_attn_1rowblock_splitkv<Kernel_traits, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_K, Is_softcap, Split, Append_KV, Is_page_attn>(params, bidb, bidh, m_block, n_split_idx, num_n_splits);
    }
}
