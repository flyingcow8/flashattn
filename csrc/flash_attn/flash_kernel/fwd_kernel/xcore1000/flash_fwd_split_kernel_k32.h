...
__forceinline__ __device__ void compute_attn_1rowblock_splitkv_k32(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {
    ...
    const int offset_swz233 = (__lane_id() / 8 % 2) ? ((__lane_id() / 16 % 2) ? -4 : 4) : 0;
    Tensor tSsK = make_tensor(make_smem_ptr(reinterpret_cast<Element *>(tSsK_ori.data().get()) + offset_swz233), tSsK_ori.layout());
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        if (masking_step > 0) {
            ...
            flash::copy_multirow_b32</*Is_even_MN=*/true, Is_even_K>(tVgV, tVrV, tVcV, params.d);
        } else {
