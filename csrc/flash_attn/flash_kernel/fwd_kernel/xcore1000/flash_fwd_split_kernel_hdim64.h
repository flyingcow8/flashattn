...
__forceinline__ __device__ void compute_attn_1rowblock_splitkv_opt_hdim64(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {
    ...
    if constexpr (Append_KV) {
        ...
        for (int n_block = n_block_max - 1; n_block >= n_block_copy_min; n_block--) {
            ...
            if (params.rotary_dim == 0) {
                ...
                if constexpr(kBlockM == kBlockN && kBlockN == 64) {//rotary only support kBlockM=kBlockN=64 now
    ...
    if (!Append_KV || params.rotary_dim == 0) {
        ...
        if constexpr(kBlockM == kBlockN && kBlockN == 64) {
            //rotary only support kBlockM=kBlockN=64 now
            if (params.is_rotary_interleaved) {
    ...
    auto gLSE_ptr = reinterpret_cast<int32_t *>(Split ? params.softmax_lseaccum_ptr : params.softmax_lse_ptr) + row_offset_lseaccum;
