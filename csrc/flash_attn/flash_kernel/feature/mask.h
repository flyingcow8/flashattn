...
struct Mask {
    ...
    __forceinline__ __device__ void apply_mask(Tensor<Engine, Layout> &tensor_,
        ...
        if constexpr (Need_masking) {
            // Reshape tensor_ from (MMA=4, MMA_M, MMA_N) to (nrow=(1, MMA_M), ncol=(4, MMA_N))
                            ...
                            for (int j = 0; j < size<1, 0>(tensor); ++j) {
                                ...
                                if constexpr (Has_alibi) {
                                    if constexpr (Is_causal) {
                                        if(custom_alibi){
                                            tensor(make_coord(i, mi), make_coord(j, nj)) -= alibi_slope * __builtin_mxc_sqrtf(abs(row_idx - col_idx));
                                        }else {
                                    ...
                                    if constexpr (Is_causal) {
                                        if(custom_alibi){
                                            if(row_idx < col_idx) {
                                                ...
                                                tensor(make_coord(i, mi), make_coord(j, nj)) -= alibi_slope * __builtin_mxc_sqrtf(row_idx - col_idx);
                                            }
                                        }else {
                                            tensor(make_coord(i, mi), make_coord(j, nj)) += alibi_slope * col_idx;
                                        }
