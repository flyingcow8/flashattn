...
struct Dropout {
    ...
    // blockN=128, waves layout: 1x4, 2x4, 1x8
    // not support hdim=192/256 bwd kernel
    template <bool encode_dropout_in_sign_bit = false, int AtomLayoutMS = 4, int AtomLayoutNS = 1, int kBlockN, typename Engine, typename Layout>
    __forceinline__ __device__ void mc_apply_dropout(Tensor<Engine, Layout> &tensor,
                                            int block_row_start, int block_col_start, int n_block) {
        using T = typename Engine::value_type;
        auto encode_dropout = [](bool keep, T val) {
            return keep ? val : (encode_dropout_in_sign_bit ? -val : T(0));
        };
        ...
        if constexpr (kBlockN == 32) {
            if constexpr (AtomLayoutMS == 4 || AtomLayoutMS == 2) {
                #pragma unroll
                for (int m = 0; m < size<1>(tensor); ++m, block_row_start += AtomLayoutMS) {
                    ...
                    for (int n = 0; n < size<2>(tensor); ++n) {
                        ...
                        #pragma unroll
                        for (int i = 0; i < 4; ++i) {
                            if (block_col_offset == 0) {
                                tensor(i, m, n) = encode_dropout(rnd_8[n * 4 + i] <= p_dropout_in_uint8_t, tensor(i, m, n));
                            } else {
                                tensor(i, m, n) = encode_dropout(rnd_8[n * 4 + i + 8] <= p_dropout_in_uint8_t, tensor(i, m, n));
                            }
                        }
                    }
                }
            }
        } else if constexpr (kBlockN == 64 || kBlockN == 128) {
            ...
            } else if constexpr (AtomLayoutNS == 8) {
                #pragma unroll
                for (int m = 0; m < size<1>(tensor); ++m, block_row_start += AtomLayoutMS) {
                    #pragma unroll
                    for (int n = 0; n < size<2>(tensor); ++n, block_col_start += 1) {
                        ...
                        #pragma unroll
                        for (int i = 0; i < 4; ++i) {
                            if (wave_col == 0 || wave_col == 4) {
                                tensor(i, m, n) = encode_dropout(rnd_8[i] <= p_dropout_in_uint8_t, tensor(i, m, n));
                            } else if (wave_col == 1 || wave_col == 5) {
                                tensor(i, m, n) = encode_dropout(rnd_8[i + 4] <= p_dropout_in_uint8_t, tensor(i, m, n));
                            } else if (wave_col == 3 || wave_col == 6) {
                                tensor(i, m, n) = encode_dropout(rnd_8[i + 8] <= p_dropout_in_uint8_t, tensor(i, m, n));
                            } else if (wave_col == 4 || wave_col == 7) {
                                tensor(i, m, n) = encode_dropout(rnd_8[i + 12] <= p_dropout_in_uint8_t, tensor(i, m, n));
                            }
                        }
                    }
