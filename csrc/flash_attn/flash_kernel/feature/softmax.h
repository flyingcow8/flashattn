...
struct Softmax {
    ...
    TensorT row_max, row_sum, tSrS_aux;
    const bool has_sink;

    __forceinline__ __device__ Softmax(const bool has_sink = false) : has_sink(has_sink){};

    template<int kBlockM, int kBlockN, typename Tensor0, typename TiledMma>
    __forceinline__ __device__ void load_s_aux(Tensor0 &gS_aux, TiledMma &tiled_mma_qk, uint32_t ngroups, uint32_t bidh, uint32_t m_block) {
        if (ngroups == 1) {
            #pragma unroll
            for(uint32_t mi = 0; mi < size(tSrS_aux); ++mi) {
                tSrS_aux(mi) = static_cast<float>(gS_aux(bidh));
            }
        } else {
            Tensor cS = cute::make_identity_tensor(Shape<Int<kBlockM>, Int<kBlockN>>{});
            auto thread_mma_qk = tiled_mma_qk.get_thread_slice(threadIdx.x);
            ...
            static_assert(decltype(size<0>(tScS_rowcol))::value == kNRows);
            #pragma unroll
            for(uint32_t mi = 0; mi < size(tSrS_aux); ++mi) {
                uint32_t row = m_block * kBlockM + get<0>(tScS_rowcol(mi, _0{}));
                uint32_t bidh_mi = (row % ngroups) + bidh * ngroups;
        ...
        if constexpr (Is_first) {
            if (has_sink) {
                ...
                for (int mi = 0; mi < size(row_max); ++mi) {
                    row_max(mi) = (tSrS_aux(mi) * float(M_LOG2E) / softmax_scale_log2);
                }
    ...
    __forceinline__ __device__ void softmax_rescale_o(Tensor0 &acc_s, Tensor1 &acc_o, Tensor2 &sRowMax, float softmax_scale_log2) {
        ...
        if constexpr (Is_first) {
            ...
            flash::sync_threads();
            if (wave_group_idx == 1) {
                row_max(0) = sRowMax(wave_group_idx ^ 1, row_offset); //lds row_max
            }
        } else {
            Tensor scores_max_prev = make_fragment_like(row_max);
            cute::copy(row_max, scores_max_prev);
    ...
    __forceinline__ __device__ void get_row_max(Tensor0 &acc_s, Tensor1 &scores_max_prev, float softmax_scale_log2) {
        Tensor scores = make_tensor(acc_s.data(), flash::convert_layout_acc_rowcol(acc_s.layout()));
        MaxOp<float> max_op;
        static_assert(decltype(size<0>(scores))::value == kNRows);
        static_assert(decltype(size<1>(scores))::value % 2 == 0);
    ...
    __forceinline__ __device__ TensorT normalize_softmax_lse(Tensor0 &acc_o, float softmax_scale, float rp_dropout=1.0) {
        ...
        for (int mi = 0; mi < size<0>(acc_o_rowcol); ++mi) {
            ...
            if (has_sink) {
                ...
                sum += __builtin_exp2f((tSrS_aux(mi) - max_scaled) * float(M_LOG2E));
            }
