...
std::vector<int64_t> get_attn_mask_stride(torch::IntArrayRef &attn_mask_shape, std::vector<int64_t> &score_shape) {
    TORCH_CHECK(score_shape.size() == 4, "score_shape must be 4-dim");
    TORCH_CHECK(attn_mask_shape.size() <= score_shape.size(), "attn_mask should have dim less than score_mask");

    int64_t accum_stride = 1;
    std::vector<int64_t> mask_stride = {0, 0, 0, 0};

    bool is_shape_valid = false;
    for (int i = 0; i < attn_mask_shape.size(); i++) {
        int64_t cur_dim_size = attn_mask_shape[attn_mask_shape.size() - 1 - i];
    ...
    TORCH_CHECK(is_shape_valid,
                "attn_mask must have shape [bs or 1, hdim or 1, q or 1, k or 1] or [hdim or 1, q or 1, k or 1] or [q "
                "or 1, k or 1] or [k or 1]");

    return mask_stride;
}

void set_params_attn_mask(Flash_fwd_params &params, bool is_causal, c10::optional<at::Tensor> &attn_mask_,
    ...
    if (attn_mask_.has_value()) {
        ...
        auto mask_stride = get_attn_mask_stride(attn_mask_shape, score_shape);
...
/*
 * @attn_mask_ -> [batch_size or 1, num_heads or 1, seqlen_q or 1, seqlen_k or 1]
 *                                 [num_heads or 1, seqlen_q or 1, seqlen_k or 1]
 *                                                 [seqlen_q or 1, seqlen_k or 1]
 *                                                                [seqlen_k or 1]
 */
std::vector<at::Tensor> mha_fwd_inference(at::Tensor &q, const at::Tensor &k, const at::Tensor &v,
    ...
    assert(false == (alibi_slopes_.has_value() && attn_mask_.has_value()));
    set_params_alibi(params, alibi_slopes_, batch_size, num_heads);
    set_params_attn_mask(params, is_causal, attn_mask_, batch_size, num_heads, seqlen_q, seqlen_k);
...
std::vector<at::Tensor> mha_varlen_fwd_inference(at::Tensor &q, const at::Tensor &k, const at::Tensor &v,
                                                 ...
                                                 const int max_seqlen_k, const float softmax_scale,
    ...
    set_params_fprop(params, batch_size, max_seqlen_q, max_seqlen_k, seqlen_q_rounded, seqlen_k_rounded, num_heads,
                     ...
                     cu_seqlens_k.data_ptr(), seqused_k.has_value() ? seqused_k.value().data_ptr() : nullptr, nullptr,
                     nullptr, p_dropout, softmax_scale, window_size_left, window_size_right, seqlenq_ngroups_swapped);
...
std::vector<at::Tensor> mha_bwd(const at::Tensor &dout, const at::Tensor &q, const at::Tensor &k, const at::Tensor &v,
                                ...
                                c10::optional<at::Tensor> &alibi_slopes_, c10::optional<at::Tensor> &attn_mask_,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("fwd_inference", &mha_fwd_inference, "Forward pass for inference only");
    m.def("varlen_fwd_inference", &mha_varlen_fwd_inference, "Forward pass (variable length)  for inference only");
