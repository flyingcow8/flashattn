...
int main()
{
    ...
    int64_t dk_size = batch_size * seqlen_k * num_heads_k * head_size_og;
    int64_t dv_size = batch_size * seqlen_k * num_heads_k * head_size_og;
    ...
    mcMalloc((void **)(&dout_ptr), dout_size * sizeof(mctlass::bfloat16_t));
    mcMalloc((void **)(&softmax_d_ptr), softmax_d_size * sizeof(float));
    mcMalloc((void **)(&softmax_lse_ptr), softmax_lse_size * sizeof(float));
    mcMalloc((void **)(&dq_ptr), dq_size * sizeof(mctlass::bfloat16_t));
    mcMalloc((void **)(&dk_ptr), dk_size * sizeof(mctlass::bfloat16_t));
    mcMalloc((void **)(&dv_ptr), dv_size * sizeof(mctlass::bfloat16_t));
    ...
    if(ret != MCFLASHATTN_STATUS_SUCCESS){
        std::cerr << "mha_bwd failed, ret:" << int(ret) << std::endl;
    } else {
        std::cerr << "mha_bwd end" << std::endl;
    }
